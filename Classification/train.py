...
model.to(device)
# model = OurEfficientNet(version='b5', num_classes=1000, pretrained=True).to(device)
# learning parameters
batch_size = 16
...
# load data
data = pd.read_csv('Classification\dataset\styles_cleaned.csv')
...
def train(model, dataloader, optimizer, loss_fn, dataset, device):
    ...
    for i, data in tqdm(enumerate(dataloader), total=int(len(dataset)/dataloader.batch_size)):
        ...
        outputs = model(image)
        targets = (gender, sub, article, color, usage)
...
for epoch in range(epochs):
    ...
    save_model(epochs, model, optimizer, criteria, name = 'model_resnet_latest.pth')
