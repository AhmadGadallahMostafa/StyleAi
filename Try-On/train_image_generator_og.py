...
def train_generator(data_loader_train, condition_generator, image_generator, discriminator, epochs, data_loader_val, writer, batch_size):
    ...
    for step in tqdm(range(17039 + 10500 + 20899, 200000)):
        ...
        with torch.no_grad():
            ...
            # the inuut needs to be removed from cuda so we can run it on cpu
            cloth_resized = F.interpolate(cloth, size=(256, 192), mode='bilinear', align_corners=True).cpu()
            cloth_mask_resized = F.interpolate(cloth_mask, size=(256, 192), mode='nearest').cpu()
            dense_pose_resized = F.interpolate(dense_pose, size=(256, 192), mode='bilinear', align_corners=True).cpu()
            parse_agnostic_resized = F.interpolate(parse_agnostic, size=(256, 192), mode='nearest').cpu()
            ...
            # Return to cuda
            fake_map = fake_map.cuda()
            warped_cloth = warped_cloth.cuda()
            warped_cloth_mask = warped_cloth_mask.cuda()
            flow_list = [flow.cuda() for flow in flow_list]
        ...
        # resizing the parse map to fit in memory
        new_parse_map_reduced = F.interpolate(new_parse_map, size=(512, 384), mode='nearest')
        ...
        with torch.no_grad():
            fake_imageD = image_generator(torch.cat((agnostic_image, dense_pose, warped_cloth), dim = 1), new_parse_map).detach()
            fake_imageD.requires_grad_(True)

        # now we will call the discriminator to get the output
        pred_fake_1, pred_fake_2 = discriminator(torch.cat((fake_imageD, new_parse_map_reduced), dim=1))
        ...
        if step % 1 == 0:
            for batch in range(batch_size):
                cloth_reduced =F.interpolate(cloth, size=(512, 384), mode='nearest')
                grid = make_image_grid([real_image_reduced[batch].cpu(), cloth_reduced[batch].cpu(), fake_image[batch].cpu()], nrow=3)
                save_image(grid, f'./Try-On/image_gen_output_og/{step}_{batch}.png')
        ...
        if (step + 1) % 1000 == 0:
            ...
            torch.save(image_generator.state_dict(),'image_generator_og.pth')
            torch.save(discriminator.state_dict(), 'discriminator_image_generator_og.pth')        
...
def main():
        ...
        # Print number of parameters in image generator
        print('Number of parameters in image generator: {}'.format(sum(p.numel() for p in image_generator.parameters() if p.requires_grad)))
        # discriminator model
        ...
        if os.path.exists('image_generator_og.pth'):
            print('loading the image generator model')
            image_generator.load_state_dict(torch.load('image_generator_og.pth'))
        if os.path.exists('discriminator_image_generator_og.pth'):
            print('loading the discriminator model')
            discriminator.load_state_dict(torch.load('discriminator_image_generator.pth'))
        

        train_generator(data_loader_train, condition_generator, image_generator, discriminator, opt.epochs, data_loader_val, None, opt.batch_size)
